...
import { Aside, Icon, Steps, TabItem, Tabs } from '@astrojs/starlight/components'
...
**Tab Mix Plus**, or any other _legacy_ extension, you have to install 3rd party helper scripts.<br/>
:::

:::tip
If you are using **Waterfox** browser, you don't need to install the `firefox-scripts` as it is already included in the browser.
:::
...
It is recommended to check periodically, or at least with every Firefox update, if new `Firefox Scripts` are available.
You should download the latest version of the scripts from the links below.

- Download [configuration files][FX_FOLDER]
- Download [utils][UTILS_EXTENSIONS_AND_SCRIPTS]

<Aside type="caution">
  The original scripts were created by [@xiaoxiaoflood](https://github.com/xiaoxiaoflood), but the
  [firefox-scripts repository](https://github.com/xiaoxiaoflood/firefox-scripts) is no longer
  maintained, and the code may be outdated.
  <br /> Always use firefox scripts from the links above.
</Aside>
...
Copy [configuration files][FX_FOLDER] to
...
1. Locate your **Profile Folder** as described in the previous step.

2. Create `chrome` folder in your **Profile Folder** (if one does not exist).

3. Extract
   [utils][UTILS_EXTENSIONS_AND_SCRIPTS]
...
![utils folder](https://private-user-images.githubusercontent.com/3650909/593565037-33cae661-732d-415c-9130-138189328e4f.png?jwt=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..YsjjupHmDQeEUTxczUgCb1S4DuSLH6UGWqBP3VxV350)
...
Plus** _development build_ (tags with **pre** or **test-build** in the title). Note that the latest
development build is compatible with all supported versions of of Firefox. (see [Browser
...
4. The installation process should begin automatically, once it complete you should restart firefox.
   :::note
...
[FX_FOLDER]: https://github.com/onemen/TabMixPlus/releases/download/dev-build/fx-folder.zip
[UTILS_EXTENSIONS_AND_SCRIPTS]: https://github.com/onemen/TabMixPlus/releases/download/dev-build/utils.zip
