---
title: New Tabs
tableOfContents: false
group: events
sidebar:
  order: 4.1
---

import eventsNewTabs from '@assets/events-new-tabs.png'
import openTabNext from '@assets/open-tab-next.png'
import switchToTab from '@assets/switchtotab.png'
import ShowInZen from '@components/ShowInZen.astro'
import { Aside } from '@starlight/components'
import { Image } from 'astro:assets'

##### Controls which URL loads in New Tab and location of newly opened tabs.<br />

<br />

<ShowInZen>
  <Aside type="tip" title="Zen Browser Compatibility: URL Bar Behavior">
    If you are using the **Zen Browser**, you may notice that opening a **New Tab** (via the `+` button or `Ctrl`/`Cmd` + `T`) defaults to focusing the **Floating URL Bar** instead of the standard New Tab Page.<br />
    Read Zed docs for more details on this behavior: [Zed Documentation](https://docs.zen-browser.app/user-manual/urlbar).

    To choose between the Zen "floating" mechanism and the legacy Tab Mix Plus behavior:

    - **Enable Legacy New Tab**: If you prefer the standard Firefox/Tab Mix Plus New Tab experience, you can disable the floating URL bar override.
      - Set `zen.urlbar.replace-newtab` to `false` in `about:config`.
      - After this change, clicking the New Tab button will open the legacy New Tab page as configured in your Tab Mix Plus options.

    - **Zen Floating URL Bar**: To keep Zen’s native behavior where the URL bar is immediately ready for input upon opening a new tab, ensure `zen.urlbar.replace-newtab` is set to `true`.

  </Aside>
</ShowInZen>

<Image class="help-img" src={eventsNewTabs} alt="image" />

###### New Tabs

- Load on New Tabs (select one):

  :::note[no-title]
  You can open `New Tab` from:
  - [New Tab Page](/help/display/display-tab-bar)
  - Menu Bar: File &gt; New Tab
  - Tab context menu: New Tab
  - Keyboard shortcut: Ctrl/Cmd + T

  See next sections about the preference that control all other tabs opening
  :::
  - **_New Tab Page (about\:newtab)_** - see more details in
    [Firefox Help](http://support.mozilla.org/en-US/kb/new-tab-page-show-hide-and-customize-top-sites)
  - Blank Page (about\:blank)
  - Home Page (to select your Home Page, you must specify a Home Page in the Firefox
    Options/General)
  - Current Page (whatever page is currently displayed in your browser)
  - Duplicate Tab with history (copy the current tab with history)

- Focus content when loading non blank page, (when not checked, focus is on the address bar)
- Open New Tab next to current one (default, unchecked, **_open new tab at the end of tabbar_**)
  - When current tab is in a group:<br/>
    If your current tab is in a group, available since Firefox 135, you can choose where to add the new tab:
    - Add to the group after current tab
    - Add after the group
    - Add at end of tabbar

---

###### Other Tabs

- Open other tabs next to current one
  - Only if related to current tab
  - Change opening order - tabs will open to the right of the most recently opened. Please refer to
    the example below.

    :::note[no-title]

    <span>
      <u>Example of changing opening order</u>
    </span>
    `[a][b][c]` are the current opened tabs with `[a]` being the current tab. <br /> `[1][2][3]` are
    the new tabs opened in that order. <br /> `[a][b][c][1][2][3]` -&gt; `[a][3][2][1][b][c]`, this
    is how tabs open without _Change opening order_. <br /> `[a][b][c][1][2][3]` -&gt;
    `[a][1][2][3][b][c]`, this is how tabs open with _Change opening order_ option checked. :::

  :::note[no-title]

  <div>
    <u>Since Firefox 156</u>
    <Image src={openTabNext} alt="image" />
  </div>

  The two checkboxes are independent. The _Change opening order_ option is replaced by the _Only
  if related to current tab_ checkbox, now labeled _Open Related tab next to current one (Change
  opening order)_. The selected tab is `[a]`, and `[1][2][3]` are the new tabs opened in that
  order:

  - when both checkboxes are unchecked, new tabs open at the end of the tab bar regardless of the
    selected tab: `[a]bc` -&gt; `[a]bc123`
  - _Open other tabs next to current one_ checked, _Open Related tab next to current one (Change
    opening order)_ unchecked - each new tab opens directly next to the current tab:
    `[a]bc` -&gt; `[a]321bc`
  - _Open Related tab next to current one (Change opening order)_ checked, with or without _Open
    other tabs next to current one_ - related tabs keep their opening order:
    `[a]bc` -&gt; `[a]123bc`

  Tabs that are not related to the current tab open at the end of the tab bar when _Open other
  tabs next to current one_ is unchecked.
  :::

- **_Open duplicated tabs next to original_**

- Move tab from 'Switch to tab' next to current one

  :::note[no-title]
  When using 'Switch to tab' from address bar popup, move the tab you switching to next to the
  currently selected tab. <br /> This only applied when the tab you switching to is in the same
  window. <Image src={switchToTab} alt="image" />
  :::

---

###### Groups of bookmarks/history

:::note[no-title]
When opening many tabs from bookmarks group or history loading time for all tabs can be long,
especially when multi-process (aka E10s, [Electrolysis](https://wiki.mozilla.org/Electrolysis)) is
enabled. <br /> It is recommended to enable these options when opening many tabs.
:::

- Load tabs progressively - load tabs no more than 3 tabs at a time.

- Don't load tabs until selected - all background tabs remain unloaded until selected.

:::note[no-title]
For both options you can set the amount of tabs that serve as a threshold to enable the option.
:::

---

###### Lock tabs

- Lock New tabs
- Lock App tabs
- **_Apply changes to open tabs_**

:::note[no-title]
You can set Tab Mix Plus to lock every new tab and every new App tab. If "Apply changes to open
tabs" is enabled, any change to <b>Lock New tabs</b> or

<b>Lock Apps tabs</b> preferences change the lock status of open tabs accordingly. :::

More about [Protected Locked and Frozen Tabs](/help/events/frozen_tabs)

[no-title]: #
