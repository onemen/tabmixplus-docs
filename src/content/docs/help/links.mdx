...
import newTabFromApp from '@assets/new_tab_from_app.png'
...
- Open links from other applications in (select one):
  ...
  - **_New tab_** - creates a new tab in the current browser window
  - Current tab - replaces existing URL page in the current browser tab with the link contents
  - New Tab After Current - new option since Firefox 146. Creates a new tab after the current tab.
    <br />Alternatively, you can achieve the same result by selecting **_New tab_** here and enabling `Open New Tab next to current one` under `Events > New Tab`.

  <Image class="img-shadow !max-w-[70%]" src={newTabFromApp} alt="image" />
...
<div class="relative float-none m-auto aspect-square w-[85%] max-w-[535px] sm:w-[65%] xl:float-right xl:ml-5 xl:w-[45%]">
  ...
  <Image
    class="img-shadow absolute top-[12%] right-[10%] w-[55%] max-w-[376px]"
